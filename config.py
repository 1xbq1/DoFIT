...
class FedArguments:
    ...
    num_clients: Optional[List[int]] = field(default_factory=list, metadata={"help": "the number of clients"})
    # num_clients1: Optional[int] = field(default=2, metadata={"help": "the number of clients"})
    # num_clients2: Optional[int] = field(default=2, metadata={"help": "the number of clients"})
    start_round: Optional[int] = field(default=1, metadata={"help": "start round"})
    tot_round: Optional[int] = field(default=200, metadata={"help": "total round"})
    sample_clients: Optional[List[int]] = field(default_factory=list, metadata={"help": "the number of clients to sample"})
    # sample_clients1: Optional[int] = field(default=2, metadata={"help": "the number of clients to sample"})
    # sample_clients2: Optional[int] = field(default=2, metadata={"help": "the number of clients to sample"})
...
class ScriptArguments:
    ...
    model_name_or_path: Optional[List[str]] = field(default_factory=list, metadata={"help": "the model name"})
    num_scenarios: Optional[int] = field(default=1, metadata={"help": "the number of heterogeneous scenarios"})
    dataset_name: Optional[List[str]] = field(default_factory=list, metadata={"help": "the dataset name"})
    ...
    batch_size: Optional[List[int]] = field(default_factory=list, metadata={"help": "the batch size"})
    # batch_size1: Optional[int] = field(default=16, metadata={"help": "the batch size"})
    # batch_size2: Optional[int] = field(default=16, metadata={"help": "the batch size"})
    ...
    output_dir: Optional[List[str]] = field(default_factory=list, metadata={"help": "the output directory"})
    # output_dir1: Optional[str] = field(default="output", metadata={"help": "the output directory"})
    # output_dir2: Optional[str] = field(default="output", metadata={"help": "the output directory"})
    lora_path: Optional[str] = field(default="lora_path", metadata={"help": "the lora path"})
    ...
    dataset_sample: Optional[List[int]] = field(default=20000, metadata={"help": "the number of samples to use from the dataset"})
...
target_str = []
#for i in range(0,5):
#    target_str.append("layers.{i}.self_attn.q_proj")
#    target_str.append("layers.{i}.self_attn.v_proj")
#for i in range(5,32):
#    target_str.append(f"layers.{i}.mlp.down_proj")
#for i in range(0, 32, 2):
#    target_str.append(f"layers.{i}.self_attn.q_proj")
#    target_str.append(f"layers.{i}.self_attn.v_proj")

#for i in range(1, 32, 2):
#    target_str.append(f"layers.{i}.mlp.down_proj")
target_str.append("mlp.down_proj")
...
def get_training_args(script_args, new_lr, output_dir, batch_size):
...
def save_config(script_args, fed_args):
    ...
    for i in range(script_args.num_scenarios):
        ...
        output_dir = f"{script_args.output_dir[i]}/{dataset_name_split}"
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        script_args.output_dir[i] = output_dir
        ...
        with open(os.path.join(script_args.output_dir[i], "args.json"), "w") as f:
            combined_dict = {
