...
def get_clients_this_round(fed_args, round, num_clients, sample_clients):
    if (fed_args.fed_alg).startswith('local'):
        clients_this_round = [int((fed_args.fed_alg)[-1])]
    else:
        if num_clients < sample_clients:
            clients_this_round = list(range(num_clients))
        else:
            random.seed(fed_args.start_round+round)
            clients_this_round = sorted(random.sample(range(num_clients), sample_clients))
